...
def visualize_graph(graph, weight="capacity", node_weight=None, filename="graph_out.png", show_zeros=False):
    ...
    nx.draw_networkx_edges(
        graph,
        pos,
        edgelist= [(u, v) for (u, v) in graph.edges() if graph[u][v]["capacity"] != 0 or show_zeros],
        width=2,
        alpha=0.5,
        edge_color="tab:red",
    )
    edge_labels = { (u, v): graph[u][v][weight] for (u, v) in graph.edges() if graph[u][v]["capacity"] != 0 or show_zeros}
